...
### Week 6 (2026-07-20)
1. **是否需要新增「持久化跳過還原」標記**：
   - **現況**：使用者選擇「暫時不要」後，該意願不會被記住，下次冷啟動仍會再問一次
...
2. **調整免費額度上限**：
   - 將免費版使用者的可用守護額度（`freeTriggersRemaining`）上限由原來的 3 次調降為 1 次，以降低免費資源的濫用，並引導高頻需求用戶升級安心版或守護版。
3. **取消進行中任務與退額度規則**：
   - 在進行中任務卡片上新增取消按鈕。點擊後提示使用者「確定要取消這個安心守護任務嗎？取消後將不再寄送通知。」。
   - 為了保持體驗彈性並防止誤觸浪費額度，實施退額度機制：在任務建立後 5 分鐘內取消，可全額退還 1 次免費額度（上限為 1）；若超過 5 分鐘後取消，則不退還額度。此機制僅對免費方案使用者生效。雲端保管模式的任務在取消時需向 Worker 發送請求同步變更 D1 狀態為 `cancelled`。
4. **雲端資料還原三層根因驗證（2026/07/20）**：
   - 【無取消機制】已驗證通過：取消按鈕觸發isCancelled=true，背景請求提前return，DB無寫入，App安全停留首頁不需重啟
   - 【逾時涵蓋不全】已驗證通過：HTTP階段12秒timeout、RevenueCat 8秒timeout，逾時皆有log與錯誤提示彈窗，不會掛起
   - 【App啟動死循環】部分驗證通過，非死循環但有UX待優化：中途取消或關閉App後，只要本地DB為空且使用者為訂閱會員，冷啟動時會再次跳出還原詢問對話框，每次都要手動選擇「暫時不要」或「立即還原」
